...
// Create HTTP server for serving the dashboard
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.url === '/') {
      // Serve HTML dashboard from frontend directory
...
async function getTopProcesses() {
  try {
    ...
    return stdout
      ...
      .filter(Boolean)
      .map((line) => {
        // Use regex to handle variable spacing in ps output
        const match = line
          .trim()
...
// Broadcast updates to all connected clients every second
setInterval(() => {
